...
class EmbeddingLoader:
    def __init__(self, file_path, enhanced_json_path=None):
        """
    ...
    def _load_embeddings(self):
        ...
        try:
            ...
            df['embedding'] = df['embedding_str'].apply(lambda x: np.array(json.loads(x), dtype=np.float64))
    ...
    def format_prompt(self, query: str, context_items: list[dict]) -> str:
        ...
        for item in context_items:
            ...
            formatted_context += f"Content from page {page_number}: {item.get('sentence_chunk', '')}\n\n"
